...
ausgetauscht werden. Auf diese Weise konvergiert der Wissenstand aller Clusterteilnehmer sukzessive zu einer gemeinsamen Sicht auf den 
...
Sobald sowohl die ShardRegion als auch mindestens ein Ingress-Knoten verfügbar sind, erteilt der ClusterCoordinator die Freigabe 
...
Nachricht und ordnet sie anhand der Fahrer-ID der zuständigen DriverActorPersistent-Instanz zu. Der Actor verarbeitet die eingehende 
Nachricht, aktualisiert seinen Zustand, persistiert die Änderungen und sendet schließlich eine aktualisierte Fahrerdaten an den 
Monitoring-Client.

\subsection{Ablauf aus Sicht der Datenübertragung}

Der Datenfluss beginnt im Ingress-Service, der die Telemetriedaten von OpenF1 kontinuierlich abruft und in einen Akka.Streams-Graph überführt. Innerhalb 
des Stream-Graphs werden die Daten gefiltert, normalisiert und für die Weiterverarbeitung strukturiert. Durch den Einsatz von Backpressure wird verhindert, 
dass nachgelagerte Komponenten überlastet werden.

Die Übergabe an die ShardRegion erfolgt über einen ShardRegionProxy. Dieser verteilt die Nachrichten in Round-Robin-Manier an jene 
Backend-Knoten, deren ShardRegion-Instanzen aktuell bereit sind, um eingehende Nachrichten zu verarbeiten. Die ShardRegion bestimmt 
anhand der Fahrer-ID die zuständige Entity (DriverActorPersistent) und leitet die Nachricht weiter. Die Entity aktualisiert ihren persistenten 
Zustand und sendet anschließend über den registrierten Event-Handler eine aktualisierte Fahrerdatenbenachrichtigung an den Monitoring-Client. Durch diese 
Entkopplung wird sowohl Skalierbarkeit als auch Stabilität des Gesamtsystems gewährleistet.
...
ihres vorherigen Zustands kann die Actor-Instanz ihren letzten konsistenten Zustand rekonstruieren und die Verarbeitung fortsetzen, ohne dass Daten verloren 
